...
def gdisconnect():
    ...
    if result['status'] == '200':
        ...
        respose = make_response(json.dumps('Failed to revoke token for the given user'), 400)
...
def editItemOfCategory(category, item_id):
    ...
    if request.method == 'GET':
        ...
        return render_template('editItemDisplay.html', category = category, categories = categories, item = item, login_session = login_session)
    else:
        user = getUserInfo(login_session['user_id'])
        category_obj = session.query(Category).filter_by(name = category).first()
...
def deleteItemOfCategory(category, item_id):
    ...
    if request.method == 'GET':
        ...
        if item is None:
            flash('Cannot find the product entered')
            return redirect(url_for('displayItemOfCategory', category = category))     
        return render_template('deleteItemOfCategory.html', category = category, categories = categories, item = item, login_session = login_session)
    else:
        user = getUserInfo(login_session['user_id'])
        category_obj = session.query(Category).filter_by(name = category).first()
